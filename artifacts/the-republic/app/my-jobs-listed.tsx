...
export default function MyJobsListedScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.background, borderBottomColor: colors.border, paddingTop: topPad + 8 }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>Jobs Listed</Text>
        <View style={{ width: 36 }} />
      </View>
      <View style={styles.empty}>
        <Feather name="list" size={40} color={colors.mutedForeground} />
        <Text style={[styles.emptyTitle, { color: colors.foreground }]}>No jobs listed</Text>
        <Text style={[styles.emptySubText, { color: colors.mutedForeground }]}>
          Jobs you post as an employer will appear here, showing applicant count and status.
        </Text>
      </View>
    </View>
  );
