...
export default function MyJobRequestsScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.background, borderBottomColor: colors.border, paddingTop: topPad + 8 }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>Job Requests</Text>
        <View style={{ width: 36 }} />
      </View>
      <View style={styles.empty}>
        <Feather name="mail" size={40} color={colors.mutedForeground} />
        <Text style={[styles.emptyTitle, { color: colors.foreground }]}>No job requests</Text>
        <Text style={[styles.emptySubText, { color: colors.mutedForeground }]}>
          Hire requests from employers who found your profile or resume will appear here.
        </Text>
      </View>
    </View>
  );
